...
def divide_groups(data):
  ...
  first_group = data[(data['target']==0)]
  second_group = data[(data['target']==1)]
  third_group = data[(data['target']==2)]
  fourth_group = data[(data['target']==3)]
  chose = [first_group,second_group, third_group, fourth_group]

  max_difference = None
  max_difference_dataframe = None

  for df in chose:
    # Calculate the absolute difference between the two columns
    diff = df['last_speed'] - df['first_speed']
    # Check if this DataFrame has the maximum difference so far
    if max_difference is None or diff.mean() > max_difference and (diff.mean(),df) not in group:
        max_difference = diff.mean()
        max_difference_dataframe = df
    group.append((diff.mean(),df))
  return sorted( group, key = lambda x : x[0], reverse=True)[0], sorted( group, key = lambda x : x[0], reverse=True)[1], sorted( group, key = lambda x : x[0], reverse=True)[2]
...
good_total = list(map(lambda x : data['good'].iloc[x]/(data['good'].iloc[x] + data['bad'].iloc[x] + data['normal'].iloc[x]), range(len(data))))
bad_total = list(map(lambda x : data['bad'].iloc[x]/(data['good'].iloc[x] + data['bad'].iloc[x] + data['normal'].iloc[x]), range(len(data))))
normal_total = list(map(lambda x : data['normal'].iloc[x]/(data['good'].iloc[x] + data['bad'].iloc[x] + data['normal'].iloc[x]), range(len(data))))
